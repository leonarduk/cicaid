...
def _run_gh_once(args: list[str], timeout: int = GH_TIMEOUT_SECONDS) -> subprocess.CompletedProcess[str]:
    ...
    `result.returncode`.
    """
    cmd = ["gh", *args, "--repo", f"{_FALLBACK_OWNER}/{_FALLBACK_REPO}"]
    try:
...
def run_gh(args: list[str]) -> subprocess.CompletedProcess[str]:
    """Run a `gh` CLI command scoped to the resolved repo. Never raises.
...
def resolve_repo(explicit: str | None) -> tuple[str, str]:
    ...
    print(
        ...
        "Run inside a GitHub repo clone or pass --repo owner/name.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def main() -> int:
    ...
    global _FALLBACK_OWNER, _FALLBACK_REPO
    _FALLBACK_OWNER, _FALLBACK_REPO = resolve_repo(args.repo)

    logger.info(f"INFO: Fetching open Dependabot PRs for {_FALLBACK_OWNER}/{_FALLBACK_REPO}...")
