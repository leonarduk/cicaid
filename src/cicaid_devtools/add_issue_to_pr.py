...
PR descriptions are updated through GitHub's REST API rather than ``gh pr
edit`` so a classic token does not also need the unrelated ``read:org`` scope.
...
def filter_existing_labels(owner: str, repo: str, labels: list[str]) -> list[str]:
    ...
    If the repo's label list can't be fetched, returns `labels` unfiltered so the
    original (pre-fix) behaviour is the fallback.
    """
...
def update_pr_body(owner: str, repo: str, pr: PullRequest, issue_number: int) -> bool:
    """Append a `Closes #NNNN` line to the PR body via GitHub's REST API.

    ``gh pr edit`` uses GraphQL and queries project metadata in addition to
    updating the body.  That makes it require ``read:org`` for organization
    repositories even though updating a pull request body only needs repo
    access.  Calling the pull-request REST endpoint avoids that extra scope.
    """
    # GitHub represents a missing PR description as null.  fetch_open_prs
    # normalizes that value, but keep this boundary defensive for callers that
    # construct PullRequest objects directly.
    new_body = (pr.body or "").rstrip()
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(
                "GitHub API update failed for PR #%s: %s",
                pr.number,
                result.stderr.strip(),
            )
            return False
