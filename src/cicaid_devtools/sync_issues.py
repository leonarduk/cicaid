...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "This command takes no flags. It fetches all open and closed issues "
            "for the current repository and writes/updates one markdown file per "
            "open issue under ./issues, removing files for issues that are no "
            "longer open.\n\n"
            "Environment variables:\n"
            "  GITHUB_TOKEN  Personal access token used to authenticate with the "
            "GitHub API. Falls back to `gh auth token` if unset."
        ),
